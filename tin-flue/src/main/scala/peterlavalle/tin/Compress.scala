...
object Compress {
	...
	def zopfli(originalData: Array[Byte], iterations: Int, blockSplittingMax: Int): Iterable[Byte] = {
		...
		val originalArray: Array[Byte] = originalData.toArray
		...
		try {
			(0 until outSize(0)).map(outData(0)).toArray
		} catch {
			case e: Throwable =>
				throw new RuntimeException(
